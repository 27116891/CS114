...
def read_command_line_args():
    ...
    if (len(sys.argv) < (2+1)): # TODO: Replace PLACE_HOLDER_BOOLEAN with the correct condition or rewrite this if statement without it.
    ...
    if (game_mode not in range(0,2+1)):
    ...
    if (graphics_mode not in range(0,1+1)):
...
def print_airport_suitcases(suitcase_numbers_array, collected_array, allowed_to_flip_array):
    ...
    allowed_to_flip_array
        ...
        If `allowed_to_flip_array[x]` is `False`, the suitcase at position `x` will not be displayed to the player. It will simply not be printed. Make sure the flip restriction does not prevent already collected cards from being printed.
    """
    if game_over: return
    space = ' '*5
    card_edge = '%5s' + CORN + LINE * 7 + CORN
